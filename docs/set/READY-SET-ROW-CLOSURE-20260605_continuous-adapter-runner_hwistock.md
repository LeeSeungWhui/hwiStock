...
superseded_by_for_operational_claims: docs/set/READY-SET-ROW-CLOSURE-20260605_operational-automated-trading-program_hwistock.md
---

# Ready-Set Row Closure Matrix — Continuous KIS Broker-Adapter Runner

## 1. Queue Rows

| order | unit_id | unit_ref | qa_ref | row_state | allowed_first_go_scope | hard_exclusions |
| --- | --- | --- | --- | --- | --- | --- |
| 1 | HWISTOCK-UNIT-010 | `docs/units/HWISTOCK-UNIT-010_kis-adapter-continuous-runner.md` | `docs/qa/QA-HWISTOCK-UNIT-010_kis-adapter-continuous-runner.md` | go_check_passed_local_no_network | Continuous 24-hour KIS KRX broker-adapter runner, KIS broker adapter, token/session handling, broker order/cancel/fill lookup, ledger, reconciliation, operator observation-window metadata, health/alerts, systemd/user-service lifecycle, read-only status API, and evidence reports. | Operation endpoints, account-affecting orders, NXT/SOR KIS broker routing, fake broker fills/balances/PnL, AI provider calls, public/LAN exposure, direct dashboard order controls, secret printing, fixed-duration runner logic. |
...
foundation only. It is itself superseded by the 2026-06-05 operational adapter
...
The row state above does not mean an operational operation run has passed. It means
