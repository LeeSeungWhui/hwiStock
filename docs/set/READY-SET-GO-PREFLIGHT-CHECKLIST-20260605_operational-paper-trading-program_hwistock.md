...
# Go Preflight Checklist — Operational Paper Trading Program

Run this checklist immediately before every Go row in the operational paper
program queue.
...
| PF-002 | yes | Re-read current completion and row matrix | This operational queue is current authority for paper-program work. |
...
| PF-005 | yes | Confirm live trading denied | Live endpoints, real-money orders, live account login, and live credentials remain forbidden. |
...
| --- | --- |
| HWISTOCK-UNIT-011 | Identify which user systemd units will be installed/started. Starting KIS paper order runner is excluded unless a later row scopes it. |
| HWISTOCK-UNIT-016 | Verify schemas, atomic publication, idempotency keys, `NO_TRADE` sentinel, executor lock/ledger, reservation accounting, order state machine, freshness TTLs, paper-only guard, and failure-mode QA are documented before implementation Go. |
...
| HWISTOCK-UNIT-013 | Confirm UNIT-016 closure first, then verify approved source registry, KIS paper-supported intraday market-data endpoints, KRX realtime support, and schema/freshness/session/risk/reservation/conflict gates. Missing KIS/source/chart data must block intents, not invent them. |
| HWISTOCK-UNIT-014 | Confirm UNIT-016 closure first, then confirm KIS paper env file existence without printing values, market/session state, paper domain guard, idempotency storage, write-ahead log, state machine, and exact bounded order scope. |
...
| HWISTOCK-UNIT-013 | KIS paper/mock market-data read network may be scoped for price/ranking/realtime collection; KIS order network is forbidden in this row. |
| HWISTOCK-UNIT-014 | KIS paper/mock KRX broker network allowed only for bounded Go/Prove evidence; live domain denied. |
