...
| KIS API reference overview | `https://apiportal.koreainvestment.com/apiservice` | Identified REST/WebSocket mode and live/paper endpoint separation. |
...
| KIS official GitHub sample repo | `https://github.com/koreainvestment/open-trading-api` | Confirmed sample code organization for domestic stock REST/WebSocket, paper/live switching, and strategy/order examples. |
| KIS sample config | `https://github.com/koreainvestment/open-trading-api/blob/main/kis_devlp.yaml` | Confirmed live/paper key separation, HTS ID, account/product-code fields, and live/paper REST/WebSocket domains. |
| Domestic cash-order sample | `https://github.com/koreainvestment/open-trading-api/blob/main/examples_llm/domestic_stock/order_cash/order_cash.py` | Confirmed `/uapi/domestic-stock/v1/trading/order-cash`, real/paper TR IDs, and `EXCG_ID_DVSN_CD` request field. |
...
| KIS API capability matrix | `docs/sources/HWISTOCK-KIS-API-CAPABILITY-MATRIX.md` | Records paper-supported, paper-constrained, paper-unsupported, fallback, and live-verification handling for the local KIS reference files. |
...
| SOR/NXT order routing | constrained | Official sample code and local references expose `EXCG_ID_DVSN_CD` and SOR/NXT-related values, but local cash-order and modify/cancel references state paper/mock investment is KRX-only. Treat NXT/SOR as venue/session parameters in the engine, not as paper-proven broker behavior. |
| Paper/mock mode exists | pass_with_constraints | Official config and auth samples separate paper app key/secret, paper account, and paper REST/WebSocket domains. Local references support the KRX paper path, but NXT/SOR order routing and NXT/integrated realtime feeds are not paper-supported. |
...
| Realtime quote paper support | constrained | KRX realtime 체결가/호가 and realtime 체결통보 have paper-supporting references; NXT and integrated realtime quote references are marked paper-unsupported. |
| Modify/cancel paper support | constrained | Modify/cancel order supports a paper TR, but the precheck query `주식정정취소가능주문조회` is paper-unsupported. Paper simulation should use local open-order state plus daily order/fill lookup, or keep this branch disabled until a later approved smoke. |
| Holiday and sellable quantity | constrained | `국내휴장일조회` and `매도가능수량조회` are marked paper-unsupported. Holiday lookup should be cached when used, and paper sellability should derive from paper balance/local position state. |
| Paper starting budget | blocked_for_public_docs | Public docs/samples did not confirm a fixed 10,000,000 KRW balance. Verify with future paper balance evidence. |
| Current exact rate limits | blocked_for_public_docs | KIS publishes current rate-limit notices, and official GitHub warns paper REST limits are lower, but this pass did not capture the exact current numeric notice body. |
...
08:00-20:00 KST envelope when supported. Because KIS paper references do not
support NXT/SOR paper proof, the pre-approval path must keep those
...
real-account/support-confirmation path before live use.
...
- No KIS live, paper, testbed, REST, or WebSocket broker endpoint was called.
...
UNIT-009 docs-only Set: PARTIAL PASS WITH KIS PAPER CONSTRAINTS
...
- paper account number/product-code/HTS ID shape in hwiStock env,
- actual paper balance,
- exact current rate limits,
- KRX paper WebSocket realtime and order/fill notification behavior.

NXT/SOR domestic cash-order acceptance cannot be proven by the current KIS paper
references. Keep NXT/SOR as internal venue/session parameters during no-order
dry-run and KIS paper-capability handling, with broker-specific KIS NXT/SOR
