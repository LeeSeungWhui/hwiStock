...
id: RUN-20260602-unit-002-home-server-paper-runner-set
type: evidence
name: Home-server paper runner Set pass
stage: set
...
# Home-Server Paper Runner Set Pass
...
- The official one-week paper/sandbox evidence runner uses `systemd`.
...
- Calendar source hierarchy, local alert channel, and one-week paper pass
...
- `docs/units/HWISTOCK-UNIT-002_home-server-paper-runner.md` is marked `set` and
  records systemd, source-unconfigured, local-only, and no-order dry-run
  contracts.
- `docs/qa/QA-HWISTOCK-UNIT-002_home-server-paper-runner.md` is marked `set` and
