...
| --- | --- | --- | --- | --- | --- |
| `deepseek_pro_hourly_market_analysis` | top of every hour, 24h | DeepSeek Pro | `pro_hourly_input_bundle/v0` | `pro_hourly_market_analysis/v1` | soft 10m, hard 20m |
| `deepseek_flash_10m_trade_document` | every 10 minutes during active investment-mode decision window | DeepSeek Flash | `flash_10m_input_bundle/v0` | `flash_trade_document/v1` | soft 2m, hard before next bucket |
| `gpt_morning_watchlist_0715` | 07:15 KST | local Codex CLI + local browser-use + ChatGPT Pro external reviewer when scoped; SSH browser-use forbidden | `gpt_morning_prompt/v0` | `morning_watchlist/v1` or explicit `NO_TRADE` safe-block | hard before first Flash bucket |
...
`pro_hourly_market_analysis/v1` includes: theme summaries, affected symbols,
...
section. It cannot include executable order fields. Provider truncation,
generic placeholder analysis, weak source claims, empty off-session no-trade
conditions, or off-session `confirmed` market confirmation are low-utility
signals and must downgrade Flash consumption to defensive/no-`BUY_NOW` mode.

`flash_trade_document/v1` is one document per Flash 10-minute decision bucket.
...
`gpt_morning_prompt/v0` is generated by the backend runtime before the GPT Pro
browser leg and includes: prior-close/overnight/current-morning analysis digest,
source ids, themes, candidate list, questions for GPT Pro, and explicit
forbidden actions.
