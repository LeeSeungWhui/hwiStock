...
| `kis_market_or_realtime_data` | KIS market/realtime/news APIs | `approved_unit_013_paper_read_pending_proof` | broker API adapter-read only | `/home/hwi/.config/hwistock/hwistockApi.env` alias only; values never stored | sanitized metadata/snapshot fields, payload refs, hashes, watermarks; no raw secrets/account ids | UNIT-013 signal input allowlist is mode-aware: paper/mock mode enables KRX + integrated realtime trade/orderbook/market-operation plus REST volume rank, execution-strength/volume-power rank, fluctuation rank, and program-trading aggregate; real investment mode additionally enables NXT realtime trade/orderbook/market-operation. UNIT-013 must not call KIS order endpoints. SOR remains disabled unless later proof adds it. |
