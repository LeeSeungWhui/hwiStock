...
| --- | --- | --- | --- |
| `dashboard_account_summary` | Market-session independent; cache-throttled; allowed weekends/off-hours with last-known-cache fallback. Fresh cache can be reused when cash, positions, or PnL display values exist; stale runner evidence may only be shown as labeled fallback. | Read-only operator dashboard balance/PnL display with `usable_for_order_preflight=false`, `accountAsOfKst`, and `dashboardSnapshotAtKst`. | Order sizing, buy/sell preflight, risk truth, or executable account authority. |
| `trading_account_truth` / deprecated alias `kis_account_truth` | Fresh read only when an actual paper/mock order intent is under preflight and the `09:00-15:00 KST` order window plus owner approval gates are open. New code should use `trading_account_truth`; `kis_account_truth` remains only as a backward-compatible alias. | Executor cash, buyable, sellable, realized-PnL, and cancelable-order evidence immediately before an order attempt. | Background dashboard refresh, weekend/off-session status cosmetics, or replacing missing order-window approval. |
