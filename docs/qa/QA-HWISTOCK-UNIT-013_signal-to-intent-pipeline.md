...
| QA-014 | P0 | adapter-read-boundary | Enable UNIT-013 KIS adapter-read collector with broker-adapter credentials | Only market-data endpoints are attempted; order/cancel/modify endpoints remain uncalled; integrated feed is accepted as analysis authority; KRX quote/session remains execution authority; NXT realtime inputs are rejected in paper/mock mode and future live mode unless separate NXT approval and Ready-Set are present | endpoint audit |
