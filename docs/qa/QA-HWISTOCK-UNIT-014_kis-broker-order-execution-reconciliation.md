...
| QA-007 | P0 | mode-gate | Request NXT/SOR/integrated/helper branches under paper/mock and future live investment modes | Paper/mock NXT, future-live default NXT, and all SOR branches write fallback/disabled records before broker transport; integrated market-data is accepted only as analysis context and cannot authorize execution without KRX quote/session/order-window evidence | capability log |
...
| QA-010 | P0 | reservation | Feed intent where pending orders would breach dynamic exposure or holding-slot cap | Reservation gate blocks before KIS transport | risk log |
