...
| QA-006 | P0 | route/capability | Submit KRX, NXT, and SOR route requests under dry-run and KIS paper-capability modes | Routes share the same state machine; KIS paper-unsupported NXT/SOR broker branches stay disabled or explicit-fallback-only | route/capability log |
...
| QA-009 | P0 | reconciliation | Feed KIS KRX paper order/fill/balance/cancel records and unsupported helper cases | Supported events update order state; unsupported helpers use local state or explicit fallback without live-only API calls | reconciliation log |
