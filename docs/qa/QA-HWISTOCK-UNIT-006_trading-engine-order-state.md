...
| QA-006 | P0 | route/capability | Submit KRX, NXT, SOR, and AUTO_SESSION route requests under dry-run and KIS adapter-capability modes | Routes share the same state machine; SOR is normalized to KRX for foundation risk gating, AUTO_SESSION requires an explicit resolved underlying venue, and KIS adapter-unsupported NXT/SOR broker branches stay disabled or explicit-fallback-only | route/capability log |
...
| QA-008 | P0 | capability | Inspect adapter capability registry for KIS adapter | KRX order/realtime capabilities are true; NXT/SOR/integrated/helper unsupported capabilities are false and produce disabled/fallback records | capability log |
| QA-009 | P0 | reconciliation | Feed KIS KRX broker order/fill/balance/cancel fixture records and unsupported helper cases | Supported KRX adapter event shapes are represented without applying broker state changes; unsupported helpers use local state or explicit fallback without operation-only API calls | reconciliation log |
