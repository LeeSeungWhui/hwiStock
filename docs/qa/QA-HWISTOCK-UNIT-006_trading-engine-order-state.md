...
| QA-006 | P0 | route/capability | Submit KRX, NXT, SOR, and AUTO_SESSION route requests under dry-run and KIS adapter-capability modes | Routes share the same state machine; AUTO_SESSION requires an explicit resolved underlying venue; paper/mock disables NXT broker branches, real investment mode enables NXT where capability flags allow it, and SOR stays disabled/fallback-only | route/capability log |
...
| QA-008 | P0 | capability | Inspect adapter capability registry for KIS adapter | Capability flags expose paper/mock KRX plus integrated market-data/account-truth helpers, real investment KRX/NXT where supported, and SOR disabled/fallback records | capability log |
