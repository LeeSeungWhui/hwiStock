...
| QA-005 | P0 | adapter | Submit approved intent before KIS adapter approval | Intent is recorded as no-order dry-run only; no broker call, simulated fill, or fake balance is produced | adapter/network log |
| QA-006 | P0 | route/capability | Submit KRX, NXT, SOR, and AUTO_SESSION route requests under dry-run and KIS adapter-capability modes | Routes share the same state machine; AUTO_SESSION requires an explicit resolved underlying venue; paper/mock disables NXT broker branches, future live mode starts `krx_only`, NXT requires separate owner approval and Ready-Set, and SOR stays disabled/fallback-only | route/capability log |
...
| QA-008 | P0 | capability | Inspect adapter capability registry for KIS adapter | Capability flags expose integrated market-data/account-truth analysis helpers, KRX-only paper/mock execution authority, future NXT disabled until separate approval/Ready-Set, and SOR disabled/fallback records | capability log |
| QA-009 | P0 | reconciliation | Feed KIS broker order/fill/balance/cancel/account-truth fixture records and disabled branch cases | Supported adapter event shapes are represented without applying broker state changes; provider helper truth is surfaced as account truth and disabled branches do not call operation-only APIs | reconciliation log |
...
- FAIL: vague AI text becomes an order, state transitions are missing, broker
  ...
  dry-run mode, or NXT/SOR broker calls are enabled in KIS broker-adapter mode
  despite unsupported adapter references.
- BLOCKED: condition schema, dry-run record shape, adapter capability boundary,
  or KIS broker-adapter reconciliation source is not selected.
...
- KIS broker-adapter reconciliation event log using supported KRX adapter data or explicit
  fallback records.
- Network/config proof that no KIS endpoint is reachable before broker-network
  approval.
- Foundation-only smoke proving no broker/KIS path, adapter/account-affecting order path, fake
