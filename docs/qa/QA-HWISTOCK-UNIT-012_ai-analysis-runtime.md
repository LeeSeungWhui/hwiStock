...
| QA-005 | P0 | flash | Trigger Flash trade-document fixture with latest Pro file, recent 10-minute sources, KIS ranking/quote/orderbook refs, previous trade-doc refs, and portfolio/order-state refs | Flash writes one `flash_trade_document/v1` for the 10-minute bucket, caps the document's action-symbol list at 5, includes WAIT_BUY/BUY_NOW/SELL_NOW/WAIT_SELL/HOLD/NO_TRADE semantics, price/size/target/stop/valid-window/thesis/why-now/confirm/cancel refs, marks portfolio conflicts, and cannot call broker/order code | fixture output |
...
| QA-012 | P0 | morning | Run or fixture the `07:15 KST` local Codex CLI browser-use morning watchlist path, then trigger the first paper Flash bucket | Flash references `morning_watchlist/v1` with market-open plan/opening triggers/invalidation conditions or writes `NO_TRADE` with no executable intents; any SSH browser-use route fails this row | artifact |
