...
| QA-014 | P0 | adapter | Submit risk-approved order intent before KIS paper approval | Intent is recorded as no-order dry-run only; KIS/external broker, internal fake broker, simulated fill, fake balance, broker paper/demo, and live routing are rejected until approved | adapter/policy log |
...
- FAIL: a single symbol can consume full capital, an entry can proceed without a
  ...
  event-first entry can bypass chart confirmation, or unapproved broker paper/mock
