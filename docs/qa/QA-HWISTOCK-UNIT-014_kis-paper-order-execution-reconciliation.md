...
type: qa_scenario
name: KIS paper order execution and reconciliation QA
unit_refs:
  - HWISTOCK-UNIT-014
  - HWISTOCK-UNIT-016
module_refs:
...
# KIS Paper Order Execution And Reconciliation QA

## 1. Purpose

Prove that approved paper intents can flow through KIS KRX paper execution and
reconciliation without live endpoints, duplicate orders, or fake broker state.

## 2. Scenario Rows

| row_id | priority | mode | steps | expected_result | evidence |
| --- | --- | --- | --- | --- | --- |
| QA-001 | P0 | domain | Configure live/unknown KIS host | Adapter fails before transport | adapter test |
| QA-002 | P0 | no-network | Run runner without paper network enable | Status is `idle_paper_network_disabled`; no broker call occurs | CLI output |
...
| QA-004 | P0 | paper-order | Run bounded KRX paper order when market/env allow | Order succeeds or returns classified paper error; no raw secret/response stored | sanitized smoke |
| QA-005 | P0 | reconcile | Run daily order/fill/balance/buyable reconciliation | Ledger fields derive from KIS paper evidence and masked account aliases | reconciliation report |
...
| QA-011 | P0 | paper-only | Configure live account profile, unknown domain, unsupported TR ID, or unsupported venue route | Runtime aborts before network transport with sanitized paper-only guard error | startup log |
...
| QA-014 | P0 | realtime-exit | Move realtime/quote fixture through stop-loss, take-profit, and trailing-stop thresholds for a held symbol | Executor emits the appropriate paper exit decision from realtime state without waiting for the next Flash tick | execution log |

## 3. PASS / FAIL / BLOCKED Rules

- PASS: paper KRX order/reconciliation is proven or safely classified, all hard
  safety gates hold, superseded waits are canceled, realtime exits are monitored,
  and restart does not duplicate orders.
- FAIL: live domain reachable, fake broker state created, unsupported branch
...
- BLOCKED: KIS paper env unavailable, market closed for order-specific smoke, or
  current KIS paper limits/rate state prevent safe bounded execution.
