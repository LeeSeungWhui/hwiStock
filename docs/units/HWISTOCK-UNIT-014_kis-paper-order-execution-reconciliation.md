...
Turn the KIS paper runner foundation into an idempotent paper execution loop
...
- On each newly accepted trade document, cancel previous unfilled `WAIT_BUY`
  paper orders unless the new document explicitly renews the same wait and all
  gates still pass.
...
- Place KRX paper cash orders only through `openapivts.koreainvestment.com`.
- Support cancel/modify where paper-supported and explicit local fallback where
  current KIS paper matrix says unsupported.
- Run daily order/fill lookup, balance, and buyable reconciliation.
- Store masked paper order ids/account aliases only when safe.
- Calculate system cash/position/exposure/PnL from actual paper events, not fake
  broker data.
...
- Live domain calls.
- NXT/SOR paper broker routing.
- Sellable quantity or realized PnL APIs when the current paper matrix says
  local fallback.
- Real account values.
...
| --- | --- | --- | --- |
| AC-01 | P0 | Paper domain only | Live/unknown KIS hosts fail before transport. |
...
| AC-04 | P0 | KRX paper order path works or fails classified | Paper order attempts produce sanitized success/error evidence with no raw secrets. |
| AC-05 | P0 | Reconciliation is broker-evidence-backed | Ledger state derives from KIS paper order/fill/balance/buyable evidence. |
| AC-06 | P0 | Unsupported branches do not call broker | NXT/SOR/integrated/helper paper-unsupported branches are disabled/fallback-only. |
...
| AC-10 | P0 | Paper-only guard aborts before transport | Unknown/live domain, live account profile, unsupported TR ID, or unsupported venue route fails before network transport. |
...
time, paper/live domain guard, secret redaction status, and whether the market
