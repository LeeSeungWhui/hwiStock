...
| AC-03 | P0 | KRX/NXT/SOR behavior is confirmed | KRX/NXT/SOR fields are documented for operation-capable APIs; hwiStock applies paper/mock KRX-only broker routing, future live starts `krx_only` and requires separate approval/Ready-Set for NXT, and SOR disabled/fallback routing | `RUN-20260602_unit-009-kis-api-portal-verification-set.md`; `RUN-20260604_unit-009-go-check-rebaseline.md` | QA-003 | pass_with_partial_boundary |
...
| Broker adapter availability | confirmed with mode gates | Official samples and local references support adapter credentials and adapter endpoints. hwiStock now treats paper/mock as integrated market-data/account-truth analysis plus KRX-only execution, future live as `krx_only` by default with NXT only after separate approval/Ready-Set, and SOR as disabled until a future contract proves it. |
...
| KRX/NXT/SOR order field | mode-gated | Local cash-order and modify/cancel references document `EXCG_ID_DVSN_CD` values `KRX`, `NXT`, and `SOR`. hwiStock enables KRX-only execution in paper/mock mode and future live default mode; NXT needs separate approval/Ready-Set, and SOR stays disabled. |
| Modify/cancel support | mode-gated | `주식주문(정정취소)` supports adapter TR `VTTC0013U`, but `주식정정취소가능주문조회` marks mock as `모의투자 미지원`; paper/mock runtime records `skipped_provider_unsupported` and must fail closed on ambiguous local pending-order state. |
| Holiday and sellable-quantity checks | mode-gated | `국내휴장일조회`, `매도가능수량조회`, and realized-PnL lookup mark mock as `모의투자 미지원`; paper/mock runtime must not call their real TR IDs and uses local calendar plus supported balance/buyable/daily-fill evidence. SELL preflight blocks unless supported sellable truth is available. |
