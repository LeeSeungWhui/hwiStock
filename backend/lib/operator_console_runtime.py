...
DEFAULT_DASHBOARD_ACCOUNT_CACHE_TTL_SECONDS = 60
...
def intEnv(key: str, default: int, *, minValue: int = 0) -> int:
...
def _dashboardAccountCacheTtlSeconds() -> int:
    return intEnv(
        "HWISTOCK_DASHBOARD_ACCOUNT_CACHE_TTL_SEC",
        DEFAULT_DASHBOARD_ACCOUNT_CACHE_TTL_SECONDS,
        minValue=0,
    )
    ...
    payload.update(
        {
            ...
            "account_cache_status": cacheStatus or cacheMeta.get("state") or "unknown",
            "accountCacheAgeSec": cacheMeta.get("ageSec"),
            ...
            "usableForOrderPreflight": False,
            "usable_for_order_preflight": False,
            "dashboardAccountSummaryReusedForOrder": False,
            "dashboard_account_summary_reused_for_order": False,
            "orderPreflightTruthSource": "trading_account_truth",
            "refreshPolicy": {
                "cacheTtlSec": cacheMeta.get("ttlSec"),
                "displayOnly": True,
                "fallbackToLastKnownCache": True,
            },
        }
    )
    if gate:
        payload["marketSessionGate"] = gate.get("evidence_payload")
        payload["calendarContext"] = gate.get("calendar_context")
    return payload


def _cachedDashboardAccountFallback(
    cachedSummary: Optional[Dict[str, Any]],
    *,
    snapshotAt: datetime,
    gate: Optional[Dict[str, Any]],
    cacheMeta: Dict[str, Any],
...
def buildDashboardAccountSummary(
    ...
    *,
    ...
    fallback = _withDashboardAccountRuntimePolicy(
        fallback,
        snapshotAt=snapshotAt,
        gate=accountGate,
    ...
    if not boolEnv("HWISTOCK_DASHBOARD_ACCOUNT_READ_ENABLED", True):
        cachedFallback = _cachedDashboardAccountFallback(
            ...
            gate=accountGate,
            cacheMeta=cacheMeta,
            refreshStatus="skipped_disabled_by_env_cache_fallback",
            status="cached_disabled_by_env",
        )
        if cachedFallback:
    ...
    if missing:
        cachedFallback = _cachedDashboardAccountFallback(
            ...
            gate=accountGate,
            cacheMeta=cacheMeta,
            refreshStatus="blocked_missing_env_cache_fallback",
            status="cached_missing_env",
        )
        if cachedFallback:
            return cachedFallback
        runnerFallback = _runnerEvidenceDashboardAccountFallback(
    ...
    try:
        ...
        if not tokenResult.get("token_present") or not token:
            cachedFallback = _cachedDashboardAccountFallback(
                ...
                gate=accountGate,
                cacheMeta=cacheMeta,
                refreshStatus="blocked_token_missing_cache_fallback",
                status="cached_token_missing",
            )
            if cachedFallback:
                return cachedFallback
            runnerFallback = _runnerEvidenceDashboardAccountFallback(
        ...
        cachedFallback = _cachedDashboardAccountFallback(
            ...
            gate=accountGate,
            cacheMeta=cacheMeta,
            refreshStatus="provider_error_cache_fallback",
            status="cached_provider_unavailable",
        )
        if cachedFallback:
    ...
    summary = _withDashboardAccountRuntimePolicy(
        summary,
        snapshotAt=snapshotAt,
        gate=accountGate,
    ...
    *,
    ...
    return {
        ...
        "summary": {
            ...
            "accountAsOfKst": accountSummary.get("accountAsOfKst"),
            "dashboardSnapshotAtKst": accountSummary.get("dashboardSnapshotAtKst"),
            "accountRefreshAllowed": accountSummary.get("accountRefreshAllowed"),
            "accountRefreshAllowedBySession": accountSummary.get("accountRefreshAllowedBySession"),
            "accountRefreshAttempted": accountSummary.get("accountRefreshAttempted"),
            "accountRefreshStatus": accountSummary.get("accountRefreshStatus"),
            "accountCacheStatus": accountSummary.get("accountCacheStatus"),
            "accountCacheAgeSec": accountSummary.get("accountCacheAgeSec"),
            "accountCacheTtlSec": accountSummary.get("accountCacheTtlSec"),
            "dashboardAccountSummaryOnly": accountSummary.get("dashboardAccountSummaryOnly"),
            "usableForOrderPreflight": accountSummary.get("usableForOrderPreflight"),
            "dashboardAccountSummaryReusedForOrder": accountSummary.get("dashboardAccountSummaryReusedForOrder"),
            "orderPreflightTruthSource": accountSummary.get("orderPreflightTruthSource"),
        ...
        "runtime": {
            ...
            "dashboardAccountSummary": {
                ...
                "source": accountSummary.get("source"),
                "accountAsOfKst": accountSummary.get("accountAsOfKst"),
                "dashboardSnapshotAtKst": accountSummary.get("dashboardSnapshotAtKst"),
                "accountRefreshAllowed": accountSummary.get("accountRefreshAllowed"),
                "accountRefreshAllowedBySession": accountSummary.get("accountRefreshAllowedBySession"),
